...
class MainActivity : FlutterActivity() {
    ...
    override fun configureFlutterEngine(flutterEngine: FlutterEngine) {
        ...
        flutterEngine.platformViewsController.registry.registerViewFactory(
            "com.mapleling.wjplayer/mpv_surface",
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, "com.mapleling.wjplayer/exoplayer")
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, "com.mapleling.wjplayer/mpv")
        ...
        diagnosticsChannel = MethodChannel(
            flutterEngine.dartExecutor.binaryMessenger,
            "com.mapleling.wjplayer/diagnostics"
        )
            ...
            when (call.method) {
                "saveImageToGallery" -> {
                    ...
                    val name = call.argument<String>("name")
                        ?: "WJPlayer_${System.currentTimeMillis()}"
                    if (bytes == null) {
        ...
        // 播放手势直接控制当前 Activity 的系统亮度与媒体音量，而不是仅给视频
        // 叠黑色遮罩或改变播放器内部增益。亮度使用 Window 属性，不需要修改系统设置权限；
        // 音量使用 STREAM_MUSIC，与手机实体音量键保持同一数值。
        systemControlsChannel = MethodChannel(
            flutterEngine.dartExecutor.binaryMessenger,
            "com.mapleling.wjplayer/system_controls"
        )
        systemControlsChannel!!.setMethodCallHandler { call, result ->
            when (call.method) {
                "getBrightness" -> result.success(currentWindowBrightness())
                "setBrightness" -> {
                ...
                "getMediaVolume" -> result.success(currentMediaVolume())
                "setMediaVolume" -> {
    ...
    /**
     * 把截图字节保存到系统「下载」目录下的 WJPlayer 子文件夹（Download/WJPlayer）。
     * Android 10+（Q）走 MediaStore.Downloads 作用域存储，**无需任何存储权限**；
     * Android 9 及以下写入公共 Download/WJPlayer（需 WRITE_EXTERNAL_STORAGE，清单已声明 maxSdk28）。
     */
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                val values = ContentValues().apply {
                    ...
                    put(
                        ...
                        Environment.DIRECTORY_DOWNLOADS + "/WJPlayer"
                    )
                ...
                val dir = java.io.File(downloadDir, "WJPlayer").apply { mkdirs() }
    ...
    /** 把崩溃文本追加进 AppLogger 同名日志文件（…/files/wjplayer_logs/wjplayer-<date>.log）。 */
    private fun appendCrashToLog(text: String) {
        try {
            val dir = java.io.File(getExternalFilesDir(null), "wjplayer_logs")
            if (!dir.exists()) dir.mkdirs()
            val date = java.text.SimpleDateFormat("yyyy-MM-dd", java.util.Locale.US)
                .format(java.util.Date())
            val ts = java.text.SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS", java.util.Locale.US)
                .format(java.util.Date())
            java.io.File(dir, "wjplayer-$date.log")
