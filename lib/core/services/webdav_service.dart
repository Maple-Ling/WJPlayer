...
<propfind xmlns="DAV:">
  ...
  Future<void> backupApp(String appData) async {
    // 创建应用专用目录
    await createDirectory('/WJPlayer');
    await createDirectory('/WJPlayer/backups');
    
    // 生成带时间戳的备份文件名
    final timestamp = DateTime.now().toIso8601String().replaceAll(':', '-');
    final backupPath = '/WJPlayer/backups/settings_$timestamp.json';
    
    // 同时更新最新备份
    const latestPath = '/WJPlayer/backups/settings_latest.json';
