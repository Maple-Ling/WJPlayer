...
class ExoPlayerAdapter implements PlayerAdapter {
  static const _channel = MethodChannel('com.mapleling.wjplayer/exoplayer');
  ...
  Future<void> initialize({
    ...
    bool zeroCopyHwdec = false,  // Windows 专属实验开关，ExoPlayer 忽略
  }) async {
    ...
    try {
      ...
      _eventChannel = EventChannel('com.mapleling.wjplayer/exoplayer/events/$_playerId');
  ...
  bool get isBitmapSubtitle => _isBitmapSubtitle;

  @override
  Widget buildVideo() {
    if (_textureId != null) {
      // 只返回视频 Texture。字幕由播放页统一作为独立 OSD 叠加层绘制，
      // 不再嵌入 Texture 的内容矩形，避免填充/裁剪时字幕跟着视频消失。
      return RepaintBoundary(child: Texture(textureId: _textureId!));
    }
