...
///
/// 三类磁盘数据，互不混淆：
/// 1. 图片缓存 `persistent_image_cache`：海报等永久持久化，不自动过期/淘汰。
...
class CacheService {
  static const _imageCacheExpiryDaysKey = 'wjplayer_image_cache_expiry_days';
  static const _videoCacheMaxSizeMBKey = 'wjplayer_video_cache_max_size_mb';
  static const _pgsBlendModeKey = 'wjplayer_pgs_blend_mode';
  ...
  /// 图片永久缓存目录（与 PersistentNetworkImageProvider 共用同一路径）。
  /// 放在 Application Support，不按时间或容量自动删除；仅手动清除/卸载/清除数据删除。
  static Future<String> get imageCacheDirPath async {
    final support = await getApplicationSupportDirectory();
    final dir = path.join(support.path, 'persistent_image_cache');
  ...
  static Future<int> getExternalMediaCacheSize() => PersistentJsonCache.size();

  static Future<int> getTotalCacheSize() async {
    return await getImageCacheSize() +
        await getVideoCacheSize() +
        await getExternalMediaCacheSize();
  }
  ...
  static Future<void> clearAllCache() async {
    ...
    await PersistentJsonCache.clear();
