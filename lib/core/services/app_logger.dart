...
/// - 同步落盘到滚动日志文件（`<support>/logs/wjplayer.log`，超限轮转 .1），
...
class AppLogger {
  ...
  /// 获取今天的日志文件名：wjplayer-YYYY-MM-DD.log
  String _getTodayFileName() {
    final now = DateTime.now();
    final year = now.year;
    final month = now.month.toString().padLeft(2, '0');
    final day = now.day.toString().padLeft(2, '0');
    return 'wjplayer-$year-$month-$day.log';
  }
  ...
  void _cleanOldLogs(Directory logDir) {
    try {
      ...
      for (final entity in logDir.listSync()) {
        ...
        if (!name.startsWith('wjplayer-')) continue;

        // 解析日期：wjplayer-2026-06-16.log
        final match =
            RegExp(r'wjplayer-(\d{4})-(\d{2})-(\d{2})').firstMatch(name);
  ...
  Future<void> init() async {
    ...
    try {
      // 日志目录：
      // - Android 用「应用专属外部目录」(Android/data/<包名>/files/wjplayer_logs)。
      ...
      final dir = Directory('${base.path}/wjplayer_logs');
  ...
  Future<String> exportToFile() async {
    ...
    final timestamp = DateTime.now()
        ...
        .replaceAll(':', '-')
        .replaceAll('.', '-');
    final fileName = 'wjplayer_log_$timestamp.txt';
