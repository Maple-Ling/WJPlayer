...
class MpvPlayerAdapter implements PlayerAdapter {
    ...
    try {
      ...
      if (Platform.isWindows) {
        ...
        if (_pgsDecoderAvailable) {
          _logger.i(
              'MpvAdapter', '检测到完整版 libmpv，PGS/SUP 图形字幕可渲染。运行时证据: $evidence');
        } else {
          _logger.w(
            'MpvAdapter',
  ...
  int? _activeLibmpvDllBytes() {
    try {
      ...
      final dll = File('${dir.path}${Platform.pathSeparator}libmpv-2.dll');
  ...
  Future<void> _applyShaderList(List<String>? shaderRefs) async {
    ...
    _activeGlslShaderCount =
        await _readbackShaderCount(np, resolvedPaths.length);
    _logger.i(
        'MpvAdapter',
    ...
    try {
...

      {
        ...
        if (np != null) {
          ...
          // 优先请求解码器原生降混到 stereo，不支持时由 mpv swresample 兜底。
          // 这比强制保留 7.1 PCM 更兼容 Android/移动设备的双声道输出。
  ...
  Future<void> loadLibassSubtitle(String path) async {
    ...
    try {
      ...
      if (ext == 'ass' || ext == 'ssa') {
        ...
        _hasBitmapSubtitle = false;
        _usingExternalSubtitle = true;
        // Preserve external ASS/SSA styles and embedded font metadata.
      } else {
        _currentSubIsAss = false;
        _hasBitmapSubtitle = false;
        _usingExternalSubtitle = true;
  ...
  Future<void> _applySubtitleRuntimeProperties() async {
    ...
    try {
      ...
      if (_hasBitmapSubtitle) {
        ...
        // 固定在播放器 OSD 中，避免填充/裁剪时字幕随视频内容变形。
        await np.setProperty('sub-ass-override', 'no');
        await np.setProperty('sub-pos', '100');
        ...
        await np.setProperty('blend-subtitles', 'no');
        // 普通文本字幕 (SRT/VTT等) 使用 OSD 层，避免被视频裁剪区域限制。
        await np.setProperty('sub-ass', 'yes');
        await np.setProperty('sub-ass-override', 'no');
  ...
  Future<void> setSubtitleFont(String fontName) async {
    // MPV follows the media/ASS embedded font. Do not override it from app UI.
    _subtitleFont = null;
    final np = _nativePlayer;
    if (np != null) {
      await np.setProperty('sub-scale-by-window', 'yes');
  ...
  Future<void> applyZeroCopyHwdec(bool enable) async {
    ...
    try {
      ...
      _logger.i(
          'MpvAdapter', '硬解零拷贝已${enable ? '开启(d3d11va)' : '关闭(auto-copy)'}');
    } catch (e) {
  ...
  Future<void> setSpeed(double speed) async {
    ...
    if (np != null) {
      ...
      await np.setProperty('interpolation', 'no');
      await np.setProperty('hdr-compute-peak', 'no');
      await np.setProperty('framedrop', clamped > 1.0 ? 'vo' : 'no');
    }
  ...
  Future<Uint8List?> screenshot() async {
    ...
    if (np != null) {
      ...
      try {
        ...
        _logger.eWithStack('MpvAdapter', 'window 模式截图失败，回退默认截图', e, stackTrace);
      } finally {
        if (shotFile != null) {
          try {
