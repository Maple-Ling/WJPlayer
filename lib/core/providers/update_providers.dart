...
final updateAutoCheckProvider =
  ...
  return PreferenceNotifier<bool>(
    defaultValue: true,
    readValue: (prefs) => prefs.getBool('wjplayer_update_auto_check'),
    writeValue: (prefs, value) async {
      await prefs.setBool('wjplayer_update_auto_check', value);
    },
  );
});
...
final updateChannelProvider =
  ...
  return PreferenceNotifier<UpdateChannel>(
    ...
    readValue: (prefs) {
      final raw = prefs.getString('wjplayer_update_channel');
      if (raw != null) return parseUpdateChannel(raw);
      // 迁移：旧版本用 bool 记录是否含预发布。
      if (prefs.getBool('wjplayer_update_include_pre') == true) {
        return UpdateChannel.prerelease;
      }
      return null; // 走 defaultValue（稳定版）
    },
    writeValue: (prefs, value) async {
      await prefs.setString('wjplayer_update_channel', value.name);
    },
  );
});
