...
final crossServerWritebackEnabledProvider =
  ...
  return PreferenceNotifier<bool>(
    defaultValue: false,
    readValue: (prefs) => prefs.getBool('wjplayer_cross_server_writeback'),
    writeValue: (prefs, value) async {
      await prefs.setBool('wjplayer_cross_server_writeback', value);
    },
  );
});
...
final crossServerWritebackRangeProvider = StateNotifierProvider<
  ...
  return PreferenceNotifier<CrossServerWritebackRange>(
    ...
    readValue: (prefs) => crossServerWritebackRangeFromWire(
        prefs.getString('wjplayer_cross_server_writeback_range')),
    writeValue: (prefs, value) async {
      await prefs.setString('wjplayer_cross_server_writeback_range',
...
final crossServerWritebackProgressProvider =
  ...
  return PreferenceNotifier<bool>(
    defaultValue: true,
    readValue: (prefs) =>
        prefs.getBool('wjplayer_cross_server_writeback_progress'),
    writeValue: (prefs, value) async {
      await prefs.setBool('wjplayer_cross_server_writeback_progress', value);
    },
  );
});
...
String? buildWatchHistoryScopeKey(ServerConfig? server) {
  ...
  if (server.sourceKind != SourceKind.emby) {
    return '${server.id}:${server.username ?? 'source'}';
  }
