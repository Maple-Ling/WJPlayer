    ...
    if (f.studios.isNotEmpty) {
      rows.add(_pickerRow(theme, '工作室', v.studio, () async {
        ...
        if (picked != null) {
          ...
          onChanged(
              v.withStudio(name, name == null ? null : f.studioIds[name]));
        }
      }));
    }
    // 排序已移到媒体库右上角，筛选面板只保留筛选条件。
...
const List<({String label, String key})> kLibrarySortOptions = [
  (label: '入库时间', key: 'DateCreated'),
  (label: '标题排序', key: 'SortName'),
  (label: '出品年份', key: 'ProductionYear'),
  (label: '评分', key: 'ExternalRating'),
];
...
class _FacetPickerDialogState extends State<_FacetPickerDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    final filtered = q.isEmpty
        ...
        : widget.options
            .where(
                (o) => o.toLowerCase().contains(q) || pinyinOf(o).contains(q))
            .toList();
