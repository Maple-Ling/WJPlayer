...
class _ServerMatchRow extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final theme = Theme.of(context);
    final sourceEntry = match.sourceEntry;
    final urls = sourceEntry?.thumbUrl?.isNotEmpty == true
        ? <String>[sourceEntry!.thumbUrl!]
        : resolveMediaItemImageUrls(
            apiClientForItem(ref, match.item), match.item,
            maxWidth: 180);
    final epLabel =
    ...
    return InkWell(
      ...
      onTap: () {
        ...
        final entry = match.sourceEntry;
        final sourceServerId = match.sourceServerId;
        if (entry != null && sourceServerId != null) {
          final server = ref
              .read(serverListProvider)
              .where((value) => value.id == sourceServerId)
              .firstOrNull;
          if (server != null) {
          ...
          children: [
            ClipRRect(
              ...
              child: MediaImage(
                ...
                httpHeaders: sourceEntry?.thumbHeaders,
                width: 48,
                height: 68,
                fit: BoxFit.cover,
                cacheWidth: 140,
