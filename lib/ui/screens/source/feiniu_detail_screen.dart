...
class FeiniuMediaDetailScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<FeiniuMediaDetailScreen> createState() =>
      _FeiniuMediaDetailScreenState();
}

class _FeiniuMediaDetailScreenState
    extends ConsumerState<FeiniuMediaDetailScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (detail.seasons.isNotEmpty) {
        final currentSeasonGuid =
            (detail.playInfo['parent_guid'] ?? detail.item['parent_guid'])
                ?.toString();
        final current = detail.seasons
            .where((season) => _stripPrefix(season.id) == currentSeasonGuid)
            .firstOrNull;
      ...
      setState(() {
        _loading = false;
        _error = error is SourceException ? error.message : '获取飞牛详情失败: $error';
      });
    }
  }
  ...
  Future<void> _selectSeason(String seasonId,
    ...
    final preferred = episodes
        .where((episode) => episode.id == preferredEpisodeGuid)
        .firstOrNull;
    ...
    final selectedIndex =
        selected == null ? -1 : episodes.indexWhere((e) => e.id == selected.id);
    if (selectedIndex >= 0) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (!mounted || !_episodeScrollController.hasClients) return;
        const cardExtent = 210.0;
        final viewport = _episodeScrollController.position.viewportDimension;
        // 当前集放在偏左约 24% 处，右侧始终能看到未播放的下一集。
        final target = (selectedIndex * cardExtent - viewport * 0.24).clamp(
          0.0,
  ...
  int _preferredFeiniuSubtitle(List<Map<String, dynamic>> tracks) {
    const simplified = ['简体', '简中', '简日', 'chs', 'zh-hans', 'zh_cn'];
    const chinese = ['中文', '中字', 'chi', 'zho', ' zh ', 'zh-cn'];
  ...
  Future<void> _loadMedia(SourceEntry entry) async {
    ...
    try {
      ...
      setState(() {
        _media = media;
        _audioIndex = maxAudio < 0 ? 0 : _audioIndex.clamp(0, maxAudio).toInt();
        if (maxSubtitle < 0) {
          _subtitleIndex = -1;
        } else if (!_hasSavedPreferences) {
          _subtitleIndex = _preferredFeiniuSubtitle(media.subtitles);
        } else {
          _subtitleIndex = _subtitleIndex.clamp(-1, maxSubtitle).toInt();
        }
        _loadingMedia = false;
      });
    } catch (error) {
  ...
  Widget build(BuildContext context) {
    ...
    final ratingItem = _ratingMediaItem(item, title);
                    ...
                    const DecoratedBox(
                      decoration: BoxDecoration(
                        gradient: LinearGradient(
                          ...
                          colors: [Colors.transparent, Color(0x22000000), Color(0xFFF0F2F0)],
                          stops: [0.45, 0.72, 1],
                        ),
                      ),
                    ),
  ...
  MediaItem _ratingMediaItem(Map<String, dynamic> item, String title) {
    final providerIds = <String, String>{};
    void add(String key, dynamic value) {
      final text = value?.toString().trim() ?? '';
      if (text.isNotEmpty) providerIds[key] = text;
    }

    add('tmdb', item['tmdb_id'] ?? item['tmdb']);
    add('imdb', item['imdb_id'] ?? item['imdb']);
    add('douban', item['douban_id'] ?? item['douban']);
    return MediaItem(
      id: widget.entry.id,
      name: title,
      type: _isSeries ? 'Series' : 'Movie',
      providerIds: providerIds,
      productionYear: int.tryParse('${item['year'] ?? ''}'),
    );
  }

  Widget _buildFormatBadges(BuildContext context) {
    ...
    if (text.contains('dovi') ||
        ...
        text.contains('dvhe')) {
      labels.add('DV');
    } else if (text.contains('smpte2084') ||
        text.contains('hdr') ||
    ...
    return Column(
      children: [
        ...
        _optionRow(
          ...
          onTap: widget.server.lines.length > 1 ? () => _showLinePicker(context) : null,
        ),
        const SizedBox(height: 8),
        _optionRow(
          context,
        ...
        _optionRow(
          ...
          onTap: subtitles.isEmpty ? null : () => _showSubtitlePicker(context),
        ),
      ],
    );
  }
  ...
  void _showLinePicker(BuildContext context) {
    showModalBottomSheet<void>(
      ...
      builder: (context) => SafeArea(
        child: ListView.builder(
          shrinkWrap: true,
          itemCount: widget.server.lines.length,
          itemBuilder: (_, index) => RadioListTile<int>(
            value: index,
            groupValue: widget.server.activeLineIndex,
            title: Text(widget.server.lines[index].name),
            subtitle: Text(widget.server.lines[index].url,
  ...
  Widget _buildMediaCards(BuildContext context) {
    ...
    if (media == null ||
        (media.video == null &&
            media.audios.isEmpty &&
            media.subtitles.isEmpty)) {
      return const SizedBox(
        height: 120,
  ...
  Widget _buildFileCard(BuildContext context, Map<String, dynamic> file) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (path.isNotEmpty) ...[
          const SizedBox(height: 8),
          SelectableText(path, style: Theme.of(context).textTheme.bodySmall),
        ],
        if (size > 0) ...[
          const SizedBox(height: 8),
