  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Column(
      ...
      children: [
        ...
        PanelSliderRow(
          label: '显示区域',
          value: danmakuDisplayArea.clamp(0.0, 1.0),
          min: 0,
          max: 1,
          valueLabel: '${(danmakuDisplayArea * 100).round()}%',
          onChanged: (value) {
...
class _SubtitleSettingsContentState
          ...
          children: [
            ...
            if (subtitles.isNotEmpty)
              ...subtitles.map((stream) => PanelOptionTile(
                    ...
                    onTap: () => ref
                        .read(subtitleTrackProvider.notifier)
                        .state = stream.index,
                  ))
  ...
  Future<void> _translateSubtitle(
    ...
    if (source == null) {
      _toast(subtitles.isEmpty ? '该片源无字幕轨可翻译；如需无字幕生成可用 Whisper（PC）' : '已取消');
      return;
    }
  ...
  void _showCustomOffsetDialog(
    ...
    final controller =
        TextEditingController(text: ref.read(target).toStringAsFixed(1));
    showDialog(
      context: context,
...
class _EpisodeSelectorContentState
  ...
  final ScrollController _episodesController = ScrollController();
  bool _didRevealCurrent = false;
  ...
  void _playEpisode(Episode episode) {
    ...
    final mediaSourceQuery = widget.currentMediaSourceId != null &&
        ...
        : '';
    // 先保存 Router；侧栏关闭后其 BuildContext 会立刻失效，直接 context.replace
    // 会被丢弃，表现为“选集点击无反应”。
    final router = GoRouter.of(context);
    Navigator.pop(context);
    Future<void>.microtask(
      () => router.replace('/player/${episode.id}$mediaSourceQuery'),
    );
  }
  ...
  Widget _buildEpisodesList(ApiClientFactory api) {
    ...
    return episodesAsync.when(
      data: (episodes) {
        final currentIndex =
            episodes.indexWhere((e) => e.id == widget.currentEpisodeId);
        if (!_didRevealCurrent && currentIndex >= 0) {
          _didRevealCurrent = true;
          WidgetsBinding.instance.addPostFrameCallback((_) {
            if (!mounted || !_episodesController.hasClients) return;
            final viewport = _episodesController.position.viewportDimension;
            final estimated =
                _isGridView ? (currentIndex ~/ 5) * 64.0 : currentIndex * 80.0;
            final target = (estimated - viewport * 0.62).clamp(
              0.0,
              _episodesController.position.maxScrollExtent,
            );
            _episodesController.jumpTo(target);
          });
        }
        if (_isGridView) {
              ...
              return GestureDetector(
                ...
                child: Container(
                  decoration: BoxDecoration(
                    color:
                        isCurrent ? colors.selectedFill : colors.controlTrack,
                    borderRadius:
    ...
    return FutureBuilder<Map<String, String>>(
      ...
      builder: (context, snap) {
        ...
        final children = <Widget>[
          ...
          if (width != null && height != null) row('分辨率', '$width × $height'),
    ...
    return Padding(
      ...
      child: Center(
        child: SizedBox(
          ...
          height: 26,
          child:
              CircularProgressIndicator(strokeWidth: 2.4, color: colors.accent),
        ),
      ),
    );
  }
