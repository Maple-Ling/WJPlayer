...
class _AddServerScreenState extends ConsumerState<AddServerScreen>
      ...
      child: Column(
        ...
        children: [
          ProtocolAddressField(
            ...
            onProtocolChanged: (value) => _protocol = value,
          ),
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          FilledButton.icon(
            onPressed: () =>
                _importFromJson(context, ref, _importController.text),
          ...
          OutlinedButton.icon(
            onPressed: () async {
              try {
                ...
                if (mounted) {
                  AppToast.show(context, '读取文件失败: $e',
  ...
  void _importFromJson(BuildContext context, WidgetRef ref, String jsonText) {
    ...
    try {
      ...
      final urls =
          urlRegex.allMatches(jsonText).map((m) => m.group(1)!).toList();
      final names =
          nameRegex.allMatches(jsonText).map((m) => m.group(1)!).toList();
  ...
  Future<void> _addServer() async {
    ...
    try {
      final url = protocolAddressValue(_urlController, _protocol);
      ...
      if (path.isNotEmpty && path != '/') {
        ...
        final urlWithoutSlash = fullUrl.endsWith('/')
