...
class _DetailHeaderState extends ConsumerState<_DetailHeader> {
  ...
  Widget build(BuildContext context) {
    ...
    final videoUrl = (useVideoBackground &&
            widget.item.remoteTrailers != null &&
            widget.item.remoteTrailers!.isNotEmpty)
    ...
    return Stack(
      children: [
        ...
        // 左上角评分跟随影视页选择的豆瓣/TMDB/IMDb 来源。
        SafeArea(
          child: Padding(
            padding: const EdgeInsets.only(left: 12, top: 58),
            child: Align(
              alignment: Alignment.topLeft,
              child: SelectedRatingBadge(item: widget.item, compact: false),
            ),
          ),
        ),
        ...
        children: [
          if (isWatched)
            const Padding(
              ...
              child:
                  Icon(Icons.check_circle, size: 16, color: Color(0xFF5B8DEF)),
            ),
          Expanded(
            child: Text(
        ...
        children: [
          Padding(
            ...
            child: Align(
              ...
              child: MediaFormatBadges(sources: info.mediaSources, max: 8),
            ),
          ),
          PlaybackOptions(
...
class _MoviePlayButtons extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final progress = watchedFraction(
            ...
            child: _CastDeviceList(
              ...
              onDeviceSelected: (device) async {
                ...
                if (connected) {
                  ...
                  if (context.mounted) {
                    AppToast.show(
                        context, success ? '已投屏到 ${device.name}' : '投屏失败，请重试');
                  }
                } else {
                  if (context.mounted) {
